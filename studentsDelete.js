...
let studentArray = JSON.parse(studentData);
...
function startMenu() {
    ...
    console.log('3) elimina studente');
    console.log('4) esci')
...
function printMenu() {
  ...
  function deleteMenu() {  
    ...
    prompt.get(schema, deleteManager);
  }

  function deleteManager (err, result) {
    const nomeDaEliminare = result.selection
   
    /* const newArray = []
    for (let i = 0; i < studentArray.length; i++) {
        const student = studentArray[i];
        if(student.nome !== nomeDaEliminare){
            newArray.push(student)
        }
    } */

    const newArray = studentArray.filter((student)=>{
        return student.nome !== nomeDaEliminare
    })
